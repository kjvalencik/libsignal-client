    ...
    fn poll(self: Pin<&mut Self>, cx: &mut std::task::Context) -> Poll<Self::Output> {
        ...
        match state {
            ...
            JsFutureState::Waiting {
                ...
                transform: _,
                waker: None,
            } => async_context.register_future().unwrap(),
            JsFutureState::Waiting { .. } => {}
        }
...
fn fulfill_promise<T, R: JsPromiseResultConstructor>(
    ...
    let opaque_ptr = cx.argument::<JsNumber>(0)?.value(&mut cx).to_bits() as *const ();
    ...
    if let Some(future) = future.upgrade() {
        ...
        {
            let futures_valid = async_context.fulfill_future();
            async_context.run_with_context(&mut cx, |cx| {
                let result = catch_unwind(AssertUnwindSafe(move || {
                    futures_valid.unwrap();
                    transform(cx, R::make(js_result))
                }));
