...
impl JsAsyncContext {
    ...
    ///
    /// Fails on overflow. Uses Result rather than `panic!`
    /// so the caller can reason about panic safety.
    pub(crate) fn register_future(&self) -> Result<(), &'static str> {
        self.shared_state.update(|state| {
            if state.num_pending_js_futures == i32::MAX {
                Err("too many pending futures (have they been leaking?)")
            } else {
                state.num_pending_js_futures += 1;
                Ok(())
            }
        })
    }
    ...
    ///
    /// Fails if there are no pending futures. Uses Result rather than `panic!`
    /// so the caller can reason about panic safety.
    ///
    /// See also [register_future](fn@Self::register_future).
    pub(crate) fn fulfill_future(&self) -> Result<(), &'static str> {
        self.shared_state.update(|state| {
            if state.num_pending_js_futures == 0 {
                Err("no pending futures, but one was just fulfilled")
            } else {
                state.num_pending_js_futures -= 1;
                Ok(())
            }
    ...
    fn context_data_object<'a>(
        ...
        if create_if_needed && context_storage.is_a::<JsUndefined, _>(cx) {
    ...
    pub fn register_context_data<'a, T: neon::types::Value>(
        ...
        assert!(
            raw_key < 1_000_000,
            "too many context data objects (are you registering them in a loop?)"
        );
